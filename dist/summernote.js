...
(function (factory) {
      ...
      create : function (sc, so, ec, eo) {
        if (!arguments.length) { // from Browser Selection
          if (isW3CRangeSupport) { // webkit, firefox
            ...
            if($(selection.baseNode).parents('.note-editable').length == 0){
              var temp = document.getElementsByClassName('note-editable')[0];
              selection = document.createRange();
              selection.selectNodeContents(temp);
              nativeRng = selection;
            }
            ...
            var temp = document.selection;
            if($(temp).parents('.note-editable').length == 0){
              temp = document.getElementsByClassName('note-editable')[0]
            }
            var textRange = temp.createRange();
  ...
  var EventHandler = function () {
    ...
    var imgUploadChange = function(oLayoutInfo, files){
    ...
    this.attach = function (oLayoutInfo, options) {
      ...
      //add img upload event
      $('#imgUpLoad').on('change', function(){
        var files = this.files;
        imgUploadChange(oLayoutInfo,files);
      })
  ...
  var Renderer = function () {
     ...
     * @param {String} [options.dropdown]
     */
    var tplImgButton = function (sLabel, options) {
      var event = '';
      var value = options.value;
      var title = options.title;
      var className = options.className;
      ...
      return '<button style="position: relative" type="button"' +
          ' class="btn btn-default btn-sm btn-small' +
          (className ? ' ' + className : '') +
          (dropdown ? ' dropdown-toggle' : '') +
          '"' +
          ...
          ' tabindex="-1">' +
          sLabel +
          '<input style="opacity:0;z-index: 1;position: absolute;top: 0;left: 0;width: 34px;height: 30px;" id="imgUpLoad" type="file" name="files" accept="image/gif,image/jpeg,image/jp2,image/jpg,image/png" />' +
          '</button>';
    };
    /**
    ...
    var tplIconButton = function (sIconClass, options) {
      ...
      if(sIconClass.indexOf('icon-picture') != -1){
        return tplImgButton(sLabel, options);
      }
    ...
    var tplButtonInfo = {
      ...
      fontsize: function (lang, options) {
        ...
        var sLabel = '<span class="note-current-fontsize">12</span>';
        return tplButton(sLabel, {
    ...
    var tplDialogs = function (lang, options) {
      var tplImageDialog = function () {
        var body = '<h5>' + lang.image.selectFromFiles + '</h5>' +
                   '<input class="note-image-input" type="file" name="files" accept="image/gif,image/jpeg,image/jp2,image/jpg,image/png" />';
